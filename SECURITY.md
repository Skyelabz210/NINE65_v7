...
As of v5.0.0, all core FHE Context constructors (`RNSFHEContext`, etc.) perform mandatory production safety checks via `assert_production_safe_fhe_config()`. This ensures that any configuration providing less than 128-bit security will cause a panic in release builds unless the `allow_insecure` feature is explicitly enabled.

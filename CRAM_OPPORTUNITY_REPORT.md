...
[45] RESOLVED->[2] | a1-defloat | crates/nine65/src/ops/sbni.rs (deleted) | issue #68 completed the SBNI/Wassan retirement: `ops/sbni.rs` (out of the module tree since 2026-08-09, per docs/LADDER_REMOVAL.md §1) is now deleted outright rather than kept as an unreachable file, taking its f64 chi-square/mean/stddev test helpers with it — no module remains to defloat. `crates/nine65/src/entropy/wassan_noise.rs` (`WassanNoiseField`) was deleted alongside it: zero production callers, feature-gated behind non-default `shadow-entropy`, absent from the entropy guidance table in `docs/ENTROPY_MODEL.md`. `blake3 = "1"` dropped from `crates/nine65/Cargo.toml` (sbni.rs was its only consumer, per docs/LADDER_REMOVAL.md §6.6). Also resolved the §6.3 item-4 "orphaned NTT shadow plumbing" deferred follow-up: `ntt_with_shadow`/`ntt_inplace_with_shadow` had their dead `shadow: &mut Option<Vec<u64>>` capture parameter (SBNI's only feeder) removed, merged back into `ntt`/`ntt_inplace`; output is bit-identical since only the `None` path ever ran. Historical record preserved in docs/LADDER_REMOVAL.md, docs/RETIRED_MECHANISMS.md, docs/SIDE_CHANNEL_THREAT_MODEL.md T5, docs/ENTROPY_MODEL.md §3 — none of those were deleted, only corrected where they asserted the file was still present on disk | → node:a1-defloat (applied) | crates/nine65/README.md

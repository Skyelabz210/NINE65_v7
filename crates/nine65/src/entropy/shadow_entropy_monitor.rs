        ...
        #[cfg(any(feature = "sequential", not(feature = "parallel")))]
        {
            ...
            let result = ct1_list
                ...
                .map(|(ct1, ct2)| {
                    let ntt = crate::arithmetic::NTTEngine::new(self.config.q, self.config.n);
                    let encoder = BFVEncoder::new(&self.config);
                    let evaluator = BFVEvaluator::new(&ntt, &encoder, Some(&self.keys.eval_key));
                    let counter = self
                        .entropy_monitor
                        .measurement_counter
                        .fetch_add(1, Ordering::Relaxed);
                ...
                ct1_list
                    ...
                    .map_init(
                        ...
                        |(ntt, encoder), (ct1, ct2)| {
                            ...
                            let evaluator =
                                BFVEvaluator::new(ntt, encoder, Some(&self.keys.eval_key));
                            evaluator.add(ct1, ct2)
                        },
                    )
                    .collect()
...
mod tests {
    ...
    fn test_entropy_different_polys_differ() {
        ...
        assert_ne!(
            e1, e2,
            "Different polynomials should yield different entropy"
        );
    }
    ...
    fn test_adapt_threading_up_and_down() {
        ...
        assert_eq!(
            very_small_batch_threads, 1,
            "Very small batch should use 1 thread"
        );

        // Standard FHE workload should use 4 threads (optimal balance)
        let standard_batch_threads = monitor.adapt_threading(10);
        assert_eq!(
            standard_batch_threads, 4,
            "Standard FHE workload should use 4 threads"
        );
        ...
        assert!(
            very_small_batch_threads >= monitor.min_threads
                && very_small_batch_threads <= monitor.max_threads
        );
        assert!(
            standard_batch_threads >= monitor.min_threads
                && standard_batch_threads <= monitor.max_threads
        );
        assert!(
            large_batch_threads >= monitor.min_threads
                && large_batch_threads <= monitor.max_threads
        );
    }
    ...
    fn test_medium_entropy_holds_steady() {
        ...
        assert_eq!(
            before, after,
            "Medium entropy should not change thread count"
        );
    }
    ...
    fn test_decrypt_roundtrip() {
        ...
        for (orig, &dec) in messages.iter().zip(decrypted.iter()) {
            assert_eq!(
                *orig % context.config.t,
                dec,
                "Decrypt must match encrypted value"
            );
        }
    }

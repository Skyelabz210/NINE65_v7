...
fn main() {
    ...
    for corrupted_lanes in 1..=6 {
        ...
        println!("  Fault Load: {}/6 Lanes | Correctness: {}", corrupted_lanes, if success { "STABLE" } else { "BROKEN" });
    ...
    let bsk_ksk = bootstrap.generate_keys(&dual_keys.secret_key, &mut rng).unwrap();

    let mut floor_correct = true;
    for i in 1..=50 {
        floor_ct = bootstrap.bootstrap(&floor_ct, &bsk_ksk.bsk, &bsk_ksk.ksk).unwrap();
        let decrypted = ctx.decrypt_dual(&floor_ct, &dual_keys.secret_key);
        if decrypted != 42 {
            println!("  TRANSUDCTION FAILURE: Noise floor collapsed at iteration {}", i);
            floor_correct = false;
            break;
        }
    }

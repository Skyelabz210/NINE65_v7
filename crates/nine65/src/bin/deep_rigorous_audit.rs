...
fn audit_div3_mechanism() {
    ...
    let inv_a: Vec<i128> = a_res
        .iter()
        .zip(basis.iter())
        .map(|(&r, &p)| exact_transcendentals::k_elim::mod_inv(r, p).unwrap())
        .collect();

    // Step 2: (1/a) / b = 1/(a*b)
    let inv_ab: Vec<i128> = inv_a
        .iter()
        .zip(b_res.iter())
        .zip(basis.iter())
        .map(|((&ia, &r), &p)| {
    ...
    // Step 3: 1 / (1/(a*b)) = a*b
    let prod_res: Vec<i128> = inv_ab
        .iter()
        .zip(basis.iter())
        .map(|(&iab, &p)| exact_transcendentals::k_elim::mod_inv(iab, p).unwrap())
        .collect();

...
use nine65::prelude::*;
use nine65::arithmetic::rns::{U512, U256};
...
fn main() {
    ...
    let (a2_val, a2_metrics) = crt_reconstruct_parallel_with_metrics(&residues, &correct_residues, primes);
    
    // Sequential Path (Simulated Garner)
    let (seq_val, seq_metrics) = crt_reconstruct_sequential_with_metrics(&residues, &correct_residues, primes);

    println!("  Metric | A2 Parallel (Time Crystal) | Sequential Garner");
    println!("  -------|----------------------------|------------------");
    println!("  Lane-wise Corruption | {} / {} | {} / {}", a2_metrics.corrupted_lanes, primes.len(), seq_metrics.corrupted_lanes, primes.len());
    println!("  Winding Lift (K) Delta | {} | {}", a2_metrics.k_delta, seq_metrics.k_delta);
    println!("  Fault Localization | {}% | {}% ", a2_metrics.localization_pct, seq_metrics.localization_pct);
    println!("  Restoration Status | {} | {}", a2_metrics.status, seq_metrics.status);
    ...
    if a2_metrics.corrupted_lanes < seq_metrics.corrupted_lanes {
        ...
        println!("  Sequential Garner allowed the fault to ripple through {}% of the substrate.", seq_metrics.localization_pct);
...
fn crt_reconstruct_parallel_with_metrics(residues: &[u64], correct: &[u64], primes: &[u64]) -> (U512, RecoveryMetrics) {
...
fn get_bit_u512(a: U512, i: u32) -> bool {
    if i < 128 { (a.d0 >> i) & 1 == 1 }
    else if i < 256 { (a.d1 >> (i - 128)) & 1 == 1 }
    else if i < 384 { (a.d2 >> (i - 256)) & 1 == 1 }
    else { (a.d3 >> (i - 384)) & 1 == 1 }
}

fn set_bit_u512(a: &mut U512, i: u32) {
    if i < 128 { a.d0 |= 1 << i; }
    else if i < 256 { a.d1 |= 1 << (i - 128); }
    else if i < 384 { a.d2 |= 1 << (i - 256); }
    else { a.d3 |= 1 << (i - 384); }
}
...
fn crt_reconstruct_sequential_with_metrics(residues: &[u64], correct: &[u64], primes: &[u64]) -> (U512, RecoveryMetrics) {

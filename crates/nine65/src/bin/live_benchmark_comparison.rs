...
fn run_benchmark(name: &str, secure_config: SecureConfig, ops_count: usize) {
    ...
    println!("\n--------------------------------------------------");
    println!("Benchmarking: {} (N={}, Primes={})", name, config.n, config.primes.len());
    println!("--------------------------------------------------");
    ...
    for i in 0..ops_count {
        ...
        if budget.remaining_millibits() < NoiseBudget::mul_plain_cost(3, &config) {
            if let (Some(ref engine), Some(ref b_keys)) = (bootstrap.as_ref(), bootstrap_keys.as_ref()) {
                ct = engine.bootstrap(&ct, &b_keys.bsk, &b_keys.ksk).expect("Bootstrap failed");
        ...
        mul_count += 1;
        budget.consume(NoiseOpType::MulPlain, NoiseBudget::mul_plain_cost(3, &config)).ok();
    }
    ...
    println!("  Average Op Latency: {} us ({:.2} ms)", avg_op_us, avg_op_us as f64 / 1000.0);
    ...
    println!("  [Comparison] NINE65 v7 Op: {:.2} ms | TFHE-rs Proj: {:.2} ms | Speedup: {:.1}x", 
        avg_op_us as f64 / 1000.0, tfhe_proj_ms, tfhe_proj_ms / (avg_op_us as f64 / 1000.0));
}

...
fn main() {
    ...
    println!("Running {} ops with low bootstrap threshold (forced transduction)...", ops_to_run);
    
    let start = Instant::now();
    for i in 1..=ops_to_run {
        // Force bootstrap if budget < 30%
        if budget.remaining_millibits() < (budget.initial_millibits() * 3 / 10) {
            println!("  [Step {}] Transduction Triggered (Budget: {} mb)", i, budget.remaining_millibits());
            ...
            bootstrap_count += 1;
            println!("  [Step {}] Transduction Complete in {} ms", i, boot_start.elapsed().as_millis());
        }

        // Perform multiplication
        ct = ctx.mul_plain_dual(&ct, 2);
        expected = (expected * 2) % config.t;
        budget.consume(NoiseOpType::MulPlain, NoiseBudget::mul_plain_cost(2, &config)).ok();

        // Verify correctness at every step
        let dec = ctx.decrypt_dual(&ct, &dual_keys.secret_key);
        if dec != expected {
            panic!("CORRECTNESS FAILURE at step {}: expected {}, got {}", i, expected, dec);
        }
    }

...
mod tests {
    ...
    fn test_u128_bit_length_correctness() {
        ...
        assert_eq!(
            u128_bit_length(u64::MAX as u128),
            64,
            "u64::MAX has 64 bits"
        );
    ...
    fn test_kelim_capacity_proximity_large_value() {
        ...
        println!(
            "[boundary] KElim Standard: capacity_bits={}, test value_bits={}, region={:?}",
            cap_bits, report.value_bits, report.region
        );
        // At 85%, region should be Warn80 or Warn90
        assert!(
            report.region == CapacityRegion::Warn80 || report.region == CapacityRegion::Warn90,
            "value at ~85% should trigger warning, got {:?}",
            report.region
        );
    }
    ...
    fn test_anchor_capacity_bits_canonical() {
        ...
        println!(
            "[boundary] anchor_capacity_bits for secure_128: {}",
            anchor_bits
        );
        // 5 primes × ~31-32 bits = ~155-160 bits
        assert!(
            anchor_bits >= 150 && anchor_bits <= 170,
            "Expected ~159 bits for 5 canonical anchors, got {}",
            anchor_bits
        );
    }
    ...
    fn test_anchor3_capacity_bits_canonical() {
        ...
        assert!(
            anchor3_bits >= 90 && anchor3_bits <= 100,
            "Expected ~93-94 bits for first 3 canonical anchors, got {}",
            anchor3_bits
        );
    }
    ...
    fn test_max_intermediate_bits_all_configs() {
        ...
        for (name, config) in &configs {
            ...
            println!(
                "[boundary] {}: intermediate={} bits, anchor={} bits, region={:?} ({}%)",
            ...
            if *name == "secure_128" || *name == "secure_192" {
                assert!(
                    report.region <= CapacityRegion::Warn80,
    ...
    fn test_check_k_proximity_small_k() {
        ...
        println!(
            "[boundary] k_proximity (42 bits): region={:?} ({}%), anchor3={}",
    ...
    fn test_anchor_capacity_drift_secure_256() {
        ...
        println!(
            "  proximity region = {:?} ({}%)",
            report.region, report.utilization_pct
        );
    ...
    fn test_post_switch_margins_at_fhe_boundaries() {
        ...
        assert!(
            !m_healthy.is_marginal && !m_healthy.is_critical,
            "129 bits in U256: headroom={}%",
            m_healthy.headroom_pct
        );

        // A value at 246 bits in U256 → 246*100/256=96%, headroom=4% → critical
        let m_critical = post_switch_margin_bits(246, 256);
        assert!(
            m_critical.is_critical,
            "246 bits in U256: headroom={}%",
            m_critical.headroom_pct
        );

        // A value at 220 bits in U256 → 14% headroom → marginal (5-15%)
        let m_marginal = post_switch_margin_bits(220, 256);
        assert!(
            m_marginal.is_marginal,
            "220 bits in U256: headroom={}%",
            m_marginal.headroom_pct
        );

        println!(
            "[boundary] post-switch margins: healthy={}%, marginal={}%, critical={}%",

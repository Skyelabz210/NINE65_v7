...
mod tests {
    ...
    fn test_bootstrap_on_fresh_ciphertext() {
        ...
        let bootstrapped = boot.bootstrap(&ct, &boot_keys.bsk, &boot_keys.ksk)
        ...
        assert_eq!(recovered, m,
            ...
            recovered, m);
        println!("[bootstrap] fresh ciphertext bootstrap: OK (plaintext={})", recovered);
    }
    ...
    fn test_bootstrap_ksk_path_secure_192() {
        ...
        let boot_keys = boot.generate_keys_with_ksk(&work_keys.secret_key, &mut rng)
            .expect("ksk keys");
        ...
        match result {
            Ok(Ok(bootstrapped)) => {
                ...
                assert_eq!(recovered, expected,
                    ...
                    recovered, expected);
                println!("[bootstrap] Q8: secure_192 KSK bootstrap succeeded: {}² mod t = {}",
                    m, expected);
            }
            Ok(Err(e)) => {
                println!("[bootstrap] Q8 FINDING: secure_192 KSK bootstrap returned Err: {}", e);
            }
            Err(_) => {
                println!("[bootstrap] Q8 FINDING: secure_192 KSK bootstrap panics with integer \
    ...
    fn test_bootstrap_exhausted_prime_list() {
        ...
        match result {
            Ok(_) => println!("[bootstrap] secure_256 bootstrap context created (BOOTSTRAP_PRIMES sufficient)"),
            Err(e) => println!("[bootstrap] secure_256 bootstrap rejected (expected if BOOTSTRAP_PRIMES < {}): {}",
                config_256.primes.len() + 1, e),
        }
    ...
    fn test_bootstrap_all_three_paths_same_plaintext() {
        ...
        let boot_keys1 = boot.generate_keys(&work_keys1.secret_key, &mut rng1)
        ...
        let after_boot_1 = boot.bootstrap(&ct1_mul, &boot_keys1.bsk, &boot_keys1.ksk)
        ...
        let boot_keys2 = boot.generate_keys_with_ksk(&work_keys2.secret_key, &mut rng2)
            .expect("ksk keys");
        ...
        let after_boot_2 = boot.bootstrap_with_ksk(&ct2_mul, &boot_keys2.bsk, &boot_keys2.ksk)
            .expect("ksk bootstrap");
        let plain_2 = ctx.decrypt_dual(&after_boot_2, &work_keys2.secret_key);

        let expected = (m * m) % config.t;
        assert_eq!(plain_1, expected, "Circular path: got {} expected {}", plain_1, expected);
        assert_eq!(plain_2, expected, "KSK path: got {} expected {}", plain_2, expected);

        println!("[bootstrap] both paths produced correct plaintext {}", expected);
    }
    ...
    fn test_auto_bootstrap_50_depth() {
        ...
        let mut evaluator = AutoBootstrapEvaluator::new(
            &ctx, &boot, &boot_keys.bsk, &boot_keys.ksk, &work_keys.eval_key, &ctx.config,
        );
        ...
        for i in 0..50u32 {
            let next_ct = match evaluator.mul_auto(&current_ct, &current_ct) {
                Ok(ct) => ct,
                Err(e) => {
                    println!("[bootstrap] Q17 FINDING: mul_auto failed at depth {}: {}", i, e);
                    break;
                }
            };
            ...
            if i % 10 == 9 {
                ...
                if recovered != expected {
                    ...
                    println!("[bootstrap] Q17 FINDING: depth {}: got {} expected {} (MISMATCH)",
                        i + 1, recovered, expected);
                } else {
                    println!("[bootstrap] checkpoint depth {}: ok (plaintext={})", i + 1, recovered);
                }
            }
        }

        println!("[bootstrap] auto-bootstrap 50-depth complete. Bootstraps performed: {}. \
                 Total muls: {}.",
            evaluator.bootstrap_count, evaluator.total_muls);

        if let Some((depth, got, exp)) = first_failure {
            println!("[bootstrap] Q17 ANSWER: Auto-bootstrap corrupts plaintext starting at depth {}. \
                     First mismatch: got {} expected {}. \
                     Cause: needs investigation — bootstrap itself works (Q7 passes) but \
                     multi-cycle auto-bootstrap accumulates error.", depth, got, exp);
        } else {
    ...
    fn test_bootstrap_roundtrip_sampled_plaintexts() {
        ...
        for i in 0..100u64 {
            ...
            let after_boot = boot.bootstrap(&ct_mul, &boot_keys.bsk, &boot_keys.ksk)
                .expect("bootstrap");
            let recovered = ctx.decrypt_dual(&after_boot, &work_keys.secret_key);
            let expected = (m * m) % t;
            if recovered != expected {
                failures += 1;
                eprintln!("[bootstrap] FAIL at m={}: got {} expected {}", m, recovered, expected);
            }
        }

        assert_eq!(failures, 0,
            "Bootstrap roundtrip failed for {}/100 sampled plaintexts", failures);
